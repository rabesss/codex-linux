...
- Added `scripts/custom-model-catalog-setup.js`, a safe direct/local provider
  ...
  `openai`.
- Added `custom-model-catalog-setup.js inspect`, a read-only catalog inspection
  command that validates catalogs and reports per-model picker labels, provider
  routing, context, and capability warnings without writing secrets or changing
  `~/.codex/config.toml`.
- Custom-model rows in the composer model submenu are now grouped by provider
  ...
  grouping patch also recovers provider labels from shim-generated descriptions
  when upstream-normalized dropdown rows no longer expose custom metadata.
- Added `make install-custom-models`, `make package-custom-models`, and
  `profiles/custom-models/features.json` for a public custom-model package
  profile that enables the maintained custom catalog stack without
  workstation-specific browser overrides.
- Added `make install-guided`, a single first-install command that combines the
  existing system/feature wizard with dependency installation, native package
  build, and package installation.
- Documented the maintained two-repository custom-model architecture with
  `rabesss/codex-shim`: Desktop owns picker and thread lifecycle behavior;
  the shim owns the loopback catalog and Responses/provider translation.
- Documented an off-screen `agent-browser` QA workflow that launches installed
  Codex Desktop under Xvfb and connects over Electron CDP, avoiding Hyprland
  focus stealing during UI verification.
- Added public troubleshooting and Browser backend constraint guidance for
  target-blank tabs, snapshot-scoped locators, the limited Playwright surface,
  internal Chrome tabs, unsupported tab-content calls, site-status latency,
  and non-blocking telemetry failures.
- Removed the unmaintained optional feature stacks that are not part of the
  active Linux port: Agent Workspaces, Read Aloud, Read Aloud MCP, Conversation
...
- First-run runtime/plugin reconciliation is hardened for packaged installs
  where the user's shell `PATH` does not expose a host Codex CLI.
- Custom-model selector documentation now records the current visible-row
  de-duplication contract: `/api/models` should expose one row per
  `provider_display_name` plus `display_name` pair, while route-stable slugs
  remain available behind the row for saved threads and overrides.
- Documented the current custom-model display-name contract: primary model
  names stay route-neutral while provider/route provenance lives in metadata,
  preventing `CLIProxyAPI / Cursor ...` prefixes from becoming the main picker
  label after shim regeneration.
- The Linux desktop settings patch now detects the current minified
  `SettingsRow` export before injecting the Linux desktop page, fixing current
  upstream bundles where the page import alias drifted.
- Linux launch startup is serialized per app instance, process recovery no
  longer depends on a surviving warm-start socket, and process scans verify the
  launcher instance id. This prevents concurrent launches or deleted runtime
  markers from creating duplicate Desktop instances.
- The extracted-app bootstrap patch now requires Electron's single-instance
  lock on Linux unless an explicit multi-launch instance was requested, and
  required-upstream validation rejects drift at that patch point.
- Bundled Browser and Chrome clients now accept the current capability-based
  node REPL runtime without requiring the removed `nodeRepl.config` marker,
  while still requiring privileged permission and fetch methods.
- Linux launches now clear disposable Electron web caches when installed build
  metadata changes and clean up app-server/plugin descendants after Electron
  exits.
- Custom-model conversations skip first-party automatic title generation,
  avoiding unrelated official-account quota errors after successful shim turns.
- The Statsig bootstrap sanitizer now removes both object and null
  `workspace_type` values before schema validation.
- Linux settings guard macOS-only Sparkle background-download methods.

- Updated the required custom-model catalog patches for the June 10 upstream
  Desktop bundle layout. Model discovery, custom-provider routing, thread
  history, tooltips, and attachment capability gates now tolerate minifier
  identifier changes while retaining fail-closed validation.
- Updated Linux Browser MCP defaults, Chrome native-host and browser-profile
  discovery, Brave Origin profile support, and the Linux desktop settings route
  for the same upstream bundle layout.
- Updater rebuild failures now retain the complete error chain, including the
  generated rebuild log path, in status output and desktop notifications.
- Graphical package authorization now times out after ten seconds when no
  Polkit agent responds, then opens the existing terminal authorization
  fallback instead of leaving the update flow blocked indefinitely.
- Updater status now clears a failed candidate when that exact package was
  subsequently installed through a manual recovery command.
- Daemon startup and explicit checks now reconcile installed or pending update
  state before checking upstream, preventing a recovered candidate from being
  rebuilt again under a new timestamp.
- Unchanged-fingerprint and same-DMG no-update checks now clear transient
  candidate artifacts and notifications before returning to idle.
- Custom-model thread forks now preserve model, provider, session provider
  configuration, and dynamic tools. This keeps `/goal` and manual forks routed
  through `codex_shim` instead of silently losing the local provider.
- Resumed custom-model threads now refresh and forward their dynamic Browser
  tool inventory.
- Existing threads now force a provider resume when the selected model crosses
  the official/custom provider boundary, preventing the next turn from being
  sent through the stream owner for the previous model.
- Custom-model documentation now explains that context-window, compaction, and
  truncation metadata come from the companion shim catalog and must be
  regenerated there when provider metadata changes.
- The Browser-control work formerly isolated on
  `plugins/browser-control-linux` is now documented as merged into `main`.

- The Linux in-app updater bridge now matches the current public updater-manager
  contract, including updater availability, unavailability reason, download
  progress, and relaunch-notice methods. This prevents current upstream Desktop
  builds from failing during bootstrap with missing manager-method
  `TypeError`s.
- The in-app updater no longer guesses whether a graphical Polkit agent exists.
  It attempts the constrained package action first, then opens a supported
  terminal for Polkit's text authentication agent when graphical authorization
  is unavailable. This fixes false "manual install" notifications on minimal
  window-manager sessions while keeping Debian, RPM, and pacman installs behind
  the packaged Polkit policy.
- Native package upgrades now restart an already-active updater user service
  after replacing its binary, instead of leaving the daemon on the deleted
  pre-upgrade executable until the next login or manual restart.
...
- Polkit policy `com.github.codex-desktop-control.update.policy` so privileged updater installs use the desktop authentication agent (`pkexec --disable-internal-agent`) instead of falling back to a textual prompt.
