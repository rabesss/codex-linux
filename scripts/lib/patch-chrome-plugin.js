...
patchFileFirstMatch(path.join(scriptsDir, "browser-client.mjs"), {
  ...
  oldTexts: [
    ...
    {
      oldText: String.raw`var Xd=dH(pH(),fH()==="win32"?"AppData\\Local\\Google\\Chrome\\User Data":"Library/Application Support/Google/Chrome");`,
      newText: String.raw`var Xd=dH(pH(),fH()==="win32"?"AppData\\Local\\Google\\Chrome\\User Data":"Library/Application Support/Google/Chrome"),codexLinuxChromeUserDataDirectories=()=>fH()==="linux"?[dH(pH(),".config","BraveSoftware","Brave-Browser"),dH(pH(),".config","google-chrome"),dH(pH(),".config","google-chrome-beta"),dH(pH(),".config","google-chrome-unstable"),dH(pH(),".config","chromium")]:[Xd];`,
    },
  ],
...
patchFileFirstMatch(path.join(scriptsDir, "browser-client.mjs"), {
  ...
  oldTexts: [
    ...
    {
      oldText: String.raw`var Fk=async(t,e)=>{let r=ib(Xd,t,"Local Extension Settings",e);if(!yH(r))return null;let n=await bH(ib(_H(),"codex"));await gH(r,n,{recursive:!0}),await Bk(ib(n,"LOCK"));let o=new mH(n,{createIfMissing:!1,keyEncoding:"utf8",valueEncoding:"utf8"});try{await o.open();let i=await o.get("extensionInstanceId");if(!i)return null;let s=JSON.parse(i);return typeof s!="string"?null:s}finally{await o.close(),await Bk(n,{force:!0,recursive:!0})}}`,
      newText: String.raw`var Fk=async(t,e,r=Xd)=>{let n=ib(r,t,"Local Extension Settings",e);if(!yH(n))return null;let o=await bH(ib(_H(),"codex"));await gH(n,o,{recursive:!0}),await Bk(ib(o,"LOCK"));let i=new mH(o,{createIfMissing:!1,keyEncoding:"utf8",valueEncoding:"utf8"});try{await i.open();let s=await i.get("extensionInstanceId");if(!s)return null;let a=JSON.parse(s);return typeof a!="string"?null:a}finally{await i.close(),await Bk(o,{force:!0,recursive:!0})}}`,
    },
  ],
  alreadyText: ["async(t,e,r=Tc)", "async(t,e,r=Xd)", "async(e,t,r=Rd)"],
});
...
patchFileFirstMatch(path.join(scriptsDir, "browser-client.mjs"), {
  ...
  oldTexts: [
    ...
    },
    {
      ...
      newText: String.raw`SH=async(t,e)=>{let r=(await vH(t)).filter(n=>n.instanceId===e);return r.length===1?r[0]:null},vH=async t=>{let e=[];for(let r of codexLinuxChromeUserDataDirectories())try{let n=await EH(r);e.push(...await Promise.all(n.map(async o=>({...o,userDataDir:r,instanceId:await Fk(o.id,t,r).catch(i=>(ue(i),null))}))))}catch(n){ue(n)}return e},EH=async r=>{let n=xH(r,"Local State"),o=JSON.parse(await wH(n,"utf8"));return o.profile.profiles_order.map((i,s)=>{let a=o.profile.info_cache[i];return a?{id:i,name:a.name,isLastUsed:o.profile.last_used===i,orderingIndex:s,avatarUrl:a.avatar_icon}:null}).filter(i=>!!i)}`,
    },
  ],
...
patchFileFirstMatch(path.join(scriptsDir, "browser-client.mjs"), {
  ...
  oldTexts: [
    ...
    },
    {
      ...
      newText: String.raw`IH=async(t,{codexSessionId:e})=>{let r=Wt(eE),n=t.filter(i=>i.info.type==="iab"),o=AH(n,e,r);await Promise.all(n.filter(i=>!o.includes(i)).map(async({api:i})=>i.close()));let s=[...t.filter(i=>i.info.type!=="iab"),...o];return await codexLinuxRankBrowserBackends(s)},AH=(t,e,r)=>e==null?[]:t.filter(n=>n.info.metadata?.codexSessionId===e&&(r==null||n.info.metadata.codexAppBuildFlavor===r));async function codexLinuxRankBrowserBackends(e){if(jk()!=="linux")return e;let t=await Promise.all(e.map(async(r,n)=>({browser:r,index:n,userTabCount:await codexLinuxExtensionUserTabCount(r)})));return t.sort(codexLinuxBackendCompare).map(({browser:r})=>r)}function codexLinuxBackendCompare(e,t){let r=e.browser.info.type==="extension",n=t.browser.info.type==="extension";return!r||!n?e.index-t.index:codexLinuxExtensionBackendScore(t)-codexLinuxExtensionBackendScore(e)||e.index-t.index}async function codexLinuxExtensionUserTabCount(e){if(e.info.type!=="extension")return-1;try{let t=await Promise.race([e.api.getUserTabs(),new Promise((r,n)=>setTimeout(()=>n(new Error("Chrome profile tab probe timed out")),750))]);return Array.isArray(t)?t.length:0}catch(t){return ue(t),0}}function codexLinuxExtensionBackendScore(e){let t=e.userTabCount>0?1e4+e.userTabCount:0,r=e.browser.info.metadata??{};r.profileIsLastUsed==="true"&&(t+=100);let n=Number(r.profileOrdering);return Number.isFinite(n)?t-n:t}var $k=async`,
    },
  ],
...
patchFile(path.join(scriptsDir, "open-chrome-window.js"), [
  ...
  const linuxExecutableOverride = process.env.CODEX_CHROME_EXECUTABLE?.trim();
  let linuxCommand = linuxExecutableOverride || commandPath("google-chrome") || commandPath("chrome") || "google-chrome";
  if (linuxExecutableOverride) {
