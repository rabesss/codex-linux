...
function findSettingsRowExportName(source) {
  const controlRowMatch = source.match(
    /function ([A-Za-z_$][\w$]*)\(e\)\{[\s\S]*?(?:\{label[,:\w]|let\{[^}]*label)[\s\S]*?control(?::|[,=}])/u,
  );
  if (controlRowMatch != null) {
    const alias = findExportedAlias(source, controlRowMatch[1]);
    if (alias != null) {
      return alias;
    }
  }
