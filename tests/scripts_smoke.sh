...
make_wizard_feature_root() {
    ...
    mkdir -p \
        "$features_root/brave-origin-browser-control" \
        "$features_root/custom-model-catalog" \
        "$features_root/open-target-discovery"
    printf '%s\n' '{"enabled":[]}' > "$features_root/features.example.json"
    cat > "$features_root/brave-origin-browser-control/feature.json" <<'JSON'
{"id":"brave-origin-browser-control","title":"Brave Origin Browser Control","description":"Brave Origin Nightly browser-control target."}
JSON
    printf '%s\n' '# Brave Origin Browser Control' > "$features_root/brave-origin-browser-control/README.md"
    cat > "$features_root/custom-model-catalog/feature.json" <<'JSON'
{"id":"custom-model-catalog","title":"Custom Model Catalog","description":"Custom model picker catalog."}
JSON
    printf '%s\n' '# Custom Model Catalog' > "$features_root/custom-model-catalog/README.md"
    cat > "$features_root/open-target-discovery/feature.json" <<'JSON'
{"id":"open-target-discovery","title":"Open Target Discovery","description":"Linux opener target discovery."}
JSON
    printf '%s\n' '# Open Target Discovery' > "$features_root/open-target-discovery/README.md"
}
...
make_fake_browser_upstream_app() {
    ...
    mkdir -p \
        ...
        "$resources_dir/plugins/openai-bundled/plugins/browser/docs" \
        "$resources_dir/plugins/openai-bundled/plugins/browser/scripts" \
        "$resources_dir/plugins/openai-bundled/plugins/browser/skills/control-in-app-browser"
...
MD
    cat > "$resources_dir/plugins/openai-bundled/plugins/browser/docs/playwright.md" <<'MD'
## Playwright
Do not assume `locator(...).selectOption(...)` exists in this environment.
MD
    cat > "$resources_dir/plugins/openai-bundled/plugins/browser/skills/control-in-app-browser/SKILL.md" <<'MD'
---
name: control-in-app-browser
description: Control the Codex in-app browser.
---
...
make_fake_staged_bundled_plugins() {
    local app_dir="$1"
    local resources_dir="$app_dir/resources/plugins/openai-bundled"
    local plugin

    mkdir -p "$resources_dir/.agents/plugins"
    cat > "$resources_dir/.agents/plugins/marketplace.json" <<'JSON'
{"plugins":[{"name":"browser"},{"name":"chrome"},{"name":"computer-use"}]}
JSON

    for plugin in browser chrome computer-use; do
        mkdir -p "$resources_dir/plugins/$plugin/.codex-plugin"
        cat > "$resources_dir/plugins/$plugin/.codex-plugin/plugin.json" <<JSON
{"name":"$plugin","version":"0.0.0-test"}
JSON
    done
}
...
test_browser_cdp_screenshot_verifier() {
    info "Checking browser CDP screenshot verifier"
    node --test "$REPO_DIR/scripts/workstation/verify-browser-cdp-screenshot.test.js"
    python3 - "$REPO_DIR/scripts/workstation/verify-browser-control.sh" <<'PY'
import sys

source = open(sys.argv[1], encoding="utf-8").read()
if '$app_dir/.codex-linux/browser-control-cdp-screenshot.png' in source:
    raise SystemExit("browser-control verifier must not write CDP screenshots under the app install root")
if 'mktemp -d "${TMPDIR:-/tmp}/codex-browser-control-cdp.XXXXXX"' not in source:
    raise SystemExit("browser-control verifier must use a temporary CDP screenshot directory")
if 'rm -rf "$cdp_screenshot_dir"' not in source:
    raise SystemExit("browser-control verifier must clean up temporary CDP screenshots")
PY
}

test_desktop_doctor_json_smoke() {
    info "Checking installed-state doctor JSON output"
    local workspace="$TMP_DIR/desktop-doctor-json"
    local app_root="$workspace/opt/codex-desktop"
    local launcher="$workspace/usr/bin/codex-desktop"
    local doctor="$workspace/usr/bin/codex-desktop-doctor"
    local desktop_entry="$workspace/usr/share/applications/codex-desktop.desktop"
    local icon="$workspace/usr/share/icons/hicolor/256x256/apps/codex-desktop.png"
    local output="$workspace/doctor.json"

    make_fake_app "$app_root"
    mkdir -p "$(dirname "$launcher")" "$(dirname "$desktop_entry")" "$(dirname "$icon")"
    printf '%s\n' '#!/usr/bin/env bash' 'exit 0' > "$app_root/electron"
    printf '%s\n' '#!/usr/bin/env bash' 'exec /opt/codex-desktop/start.sh "$@"' > "$launcher"
    cp "$REPO_DIR/packaging/linux/codex-desktop-doctor.py" "$doctor"
    chmod +x "$app_root/electron" "$launcher" "$doctor"
...
DESKTOP
    printf '%s\n' 'png fixture' > "$icon"
    cat > "$app_root/resources/codex-linux-build-info.json" <<'JSON'
{
  "upstreamDmg": {
...
JSON
    ...
    python3 "$REPO_DIR/packaging/linux/codex-desktop-doctor.py" \
        --json \
        ...
        --icon-path "$icon" \
        --updater-path "$workspace/usr/bin/codex-update-manager" \
        --update-service-path "$workspace/usr/lib/systemd/user/codex-update-manager.service" \
        --webview-port 1 \
        --cdp-port 0 \
        --no-plugin-probes >"$output"

    node - "$output" <<'NODE' || fail "Doctor JSON did not match the expected ready minimal install"
const fs = require("node:fs");
const report = JSON.parse(fs.readFileSync(process.argv[2], "utf8"));
if (report.packageName !== "codex-desktop") {
  throw new Error(`unexpected package name: ${report.packageName}`);
}
if (report.summary.fail !== 0 || report.readiness.ready !== true) {
  throw new Error(`expected ready report with no failed checks: ${JSON.stringify(report.summary)}`);
}
const ids = new Set(report.checks.map((check) => check.id));
for (const id of ["launcher", "doctor", "app_start", "electron_runtime", "managed_node", "desktop_entry"]) {
  if (!ids.has(id)) {
    throw new Error(`missing check id: ${id}`);
  }
}
NODE
}
...
test_deb_builder_smoke() {
    ...
    make_fake_staged_bundled_plugins "$app_dir"
...
SCRIPT
    ...
    assert_file_exists "$pkg_root/opt/codex-desktop/update-builder/scripts/lib/codex-cli-runtime.sh"
    ...
    assert_file_exists "$pkg_root/opt/codex-desktop/update-builder/linux-features/custom-model-catalog/feature.json"
    ...
    assert_file_exists "$pkg_root/opt/codex-desktop/update-builder/resources/plugins/openai-bundled/plugins/browser/.codex-plugin/plugin.json"
    assert_file_exists "$pkg_root/opt/codex-desktop/update-builder/resources/plugins/openai-bundled/plugins/chrome/.codex-plugin/plugin.json"
    assert_file_exists "$pkg_root/opt/codex-desktop/update-builder/resources/plugins/openai-bundled/plugins/computer-use/.codex-plugin/plugin.json"
    ...
    assert_mode "$pkg_root/usr/bin/codex-desktop-doctor" "755"
...
test_update_builder_preserves_enabled_linux_features_config() {
    ...
    mkdir -p "$workspace"
    make_fake_app "$app_dir"
    mkdir -p "$features_root/custom-model-catalog" "$features_root/local/local-tool"
    printf '%s\n' '# Linux Features' > "$features_root/README.md"
    printf '%s\n' '{"enabled":[]}' > "$features_root/features.example.json"
    printf '%s\n' '{"id":"custom-model-catalog","title":"Custom Model Catalog"}' \
        > "$features_root/custom-model-catalog/feature.json"
    printf '%s\n' '# Custom Model Catalog' > "$features_root/custom-model-catalog/README.md"
...
JSON
    ...
    assert_contains "$staged_config" "custom-model-catalog"
...
if (JSON.stringify(config) !== JSON.stringify({ enabled: ["custom-model-catalog", "local-tool"] })) {
  process.exit(1);
}
NODE
...
SCRIPT
    ...
    assert_contains "$pkg_root/usr/bin/codex-cua-lab-doctor" 'PACKAGE_NAME_TEMPLATE = "codex-cua-lab"'
    assert_not_contains "$pkg_root/usr/bin/codex-cua-lab-doctor" "__PACKAGE_NAME__"
...
SCRIPT
    ...
    assert_file_not_exists "$pkg_root/usr/share/polkit-1/actions/com.github.codex-desktop-control.update.policy"
...
SCRIPT
    ...
    assert_file_not_exists "$capture_dir/staging/usr/share/polkit-1/actions/com.github.codex-desktop-control.update.policy"
...
SCRIPT
    ...
    assert_file_not_exists "$capture_dir/AppDir/usr/share/polkit-1/actions/com.github.codex-desktop-control.update.policy"
...
test_native_shortcut_targets_compose_existing_flows() {
    ...
    local guided_log="$TMP_DIR/make-install-guided.log"
    ...
    make -n -C "$REPO_DIR" install-guided >"$guided_log"
    assert_contains "$guided_log" 'CODEX_BOOTSTRAP_INSTALL_DEPS=1'
    assert_contains "$guided_log" 'CODEX_BOOTSTRAP_INSTALL_NATIVE=1'
    assert_contains "$guided_log" 'bash scripts/bootstrap-wizard.sh'
}
...
test_native_upgrade_respects_disabled_updater_service() {
    info "Checking native upgrades respect disabled updater services"
    local helper="$REPO_DIR/packaging/linux/codex-update-manager-user-service.sh"

    assert_contains "$helper" 'is-active "$SERVICE_NAME"'
    assert_contains "$helper" 'restart "$SERVICE_NAME"'
...
test_setup_native_wizard_noninteractive_feature_writer() {
    ...
    cat > "$config" <<'JSON'
{"enabled":["custom-model-catalog"]}
JSON

    CODEX_BOOTSTRAP_NONINTERACTIVE=1 \
    CODEX_LINUX_FEATURES_ROOT="$features_root" \
    CODEX_LINUX_FEATURES_CONFIG="$config" \
    CODEX_LINUX_FEATURES="open-target-discovery,brave-origin-browser-control" \
    CODEX_LINUX_DISABLE_FEATURES="custom-model-catalog" \
    PACKAGE_WITH_UPDATER=0 \
        bash "$REPO_DIR/scripts/bootstrap-wizard.sh" >"$output_log"

    assert_json_enabled_equals "$config" '["open-target-discovery","brave-origin-browser-control"]'
    assert_contains "$output_log" "open-target-discovery"
    assert_contains "$output_log" "brave-origin-browser-control"
...
test_setup_native_wizard_rejects_features_without_readme() {
    ...
    make_wizard_feature_root "$features_root"
    rm -f "$features_root/open-target-discovery/README.md"
    printf '%s\n' '{"enabled":[]}' > "$config"

    if CODEX_BOOTSTRAP_NONINTERACTIVE=1 \
        CODEX_LINUX_FEATURES_ROOT="$features_root" \
        CODEX_LINUX_FEATURES_CONFIG="$config" \
        CODEX_LINUX_FEATURES="open-target-discovery" \
            bash "$REPO_DIR/scripts/bootstrap-wizard.sh" >"$output_log" 2>&1; then
...
test_setup_native_wizard_rejects_conflicting_feature_ids() {
    ...
    if CODEX_BOOTSTRAP_NONINTERACTIVE=1 \
        CODEX_LINUX_FEATURES_ROOT="$features_root" \
        CODEX_LINUX_FEATURES_CONFIG="$config" \
        CODEX_LINUX_FEATURES="custom-model-catalog" \
        CODEX_LINUX_DISABLE_FEATURES="custom-model-catalog" \
            bash "$REPO_DIR/scripts/bootstrap-wizard.sh" >"$output_log" 2>&1; then
        fail "setup wizard should reject conflicting feature ids"
    fi

    assert_contains "$output_log" "Linux feature ids cannot be both enabled and disabled: custom-model-catalog"
    assert_json_enabled_equals "$config" '[]'
}
...
test_setup_native_wizard_accepts_numbered_feature_selection() {
    ...
    make_wizard_feature_root "$features_root"
    printf '%s\n' '{"enabled":["custom-model-catalog"]}' > "$config"
    ...
    assert_json_enabled_equals "$config" '["brave-origin-browser-control","open-target-discovery"]'
    assert_contains "$output_log" "1\\. \\[available\\] brave-origin-browser-control - Brave Origin Browser Control"
    assert_contains "$output_log" "2\\. \\[enabled\\] custom-model-catalog - Custom Model Catalog"
...
JSON
    ...
    assert_json_enabled_equals "$config" '["custom-model-catalog"]'
    assert_contains "$output_log" "Enabled Linux features: custom-model-catalog"
...
test_setup_native_wizard_prints_deep_readiness_guidance() {
    info "Checking setup-native wizard detailed Computer Use readiness"
    ...
    make_wizard_feature_root "$features_root"
    printf '%s\n' '{"enabled":["custom-model-catalog"]}' > "$config"
    mkdir -p "$fake_home/.config/codex-desktop"
...
test_installer_normalizes_relative_linux_features_config() {
    info "Checking installer preserves relative Linux feature config across workdir changes"
    local root="$TMP_DIR/relative-feature-config"
    mkdir -p "$root/profiles/workstation"
    printf '%s\n' '{"enabled":["custom-model-catalog"]}' > "$root/profiles/workstation/features.json"

    bash -c '
        cd "$1"
        export CODEX_INSTALLER_SOURCE_ONLY=1
        export CODEX_LINUX_FEATURES_CONFIG="profiles/workstation/features.json"
        source "$0"
        normalize_linux_features_config_path
        [ "$CODEX_LINUX_FEATURES_CONFIG" = "$1/profiles/workstation/features.json" ]
    ' "$REPO_DIR/install.sh" "$root" || fail "Expected installer to canonicalize relative Linux feature config"
}
...
test_managed_codex_cli_launcher() {
    info "Checking managed Codex CLI resource launcher"
    local workspace="$TMP_DIR/managed-codex-cli"
    local runtime_dir="$workspace/node-runtime"
    local package_root="$workspace/codex-cli"
    local launcher_path="$workspace/bin/codex"

    mkdir -p "$runtime_dir/bin" "$package_root/node_modules/@openai/codex/bin"
...
#!/usr/bin/env bash
echo "npm should not be called when package is already staged" >&2
exit 44
SCRIPT
    chmod +x "$runtime_dir/bin/node" "$runtime_dir/bin/npm"
    printf '%s\n' 'throw new Error("fake codex.js should be executed by fake node");' \
        > "$package_root/node_modules/@openai/codex/bin/codex.js"
...
test_launcher_template_sanity() {
    ...
    local missing_cli_workspace="$TMP_DIR/launcher-missing-cli"
    local missing_cli_start="$missing_cli_workspace/start.sh"
    local missing_cli_stdout="$missing_cli_workspace/stdout.log"
    local missing_cli_stderr="$missing_cli_workspace/stderr.log"
    local missing_cli_electron_log="$missing_cli_workspace/electron.log"
    local missing_cli_update_manager_log="$missing_cli_workspace/update-manager.log"
    local missing_cli_notify_log="$missing_cli_workspace/notify.log"
    local missing_cli_home="$missing_cli_workspace/home"
    local missing_cli_long_tmp="$missing_cli_workspace/very-long-temp-root/that-keeps-going/until-the-codex-ipc-unix-socket-path-would-exceed-linux-sun-path-limits"
    local missing_cli_port

    mkdir -p \
        "$missing_cli_workspace/.codex-linux" \
        "$missing_cli_workspace/content/webview" \
        "$missing_cli_home" \
        "$missing_cli_long_tmp"
    missing_cli_port="$(python3 - <<'PY'
import socket
...
PY
    ...
    cat > "$missing_cli_workspace/content/webview/index.html" <<'HTML'
<!doctype html>
<html>
<head><title>Codex</title></head>
<body><div id="startup-loader"></div></body>
</html>
HTML
    cp "$REPO_DIR/launcher/webview-server.py" "$missing_cli_workspace/.codex-linux/webview-server.py"
    cat > "$missing_cli_workspace/electron" <<'SCRIPT'
#!/usr/bin/env bash
{
    ...
    if [ -d "${TMPDIR:-}" ] && [ ! -L "${TMPDIR:-}" ]; then
        printf 'TMPDIR_TYPE=private-directory\n'
    else
        printf 'TMPDIR_TYPE=unsafe\n'
    fi
} >> "$CODEX_MISSING_CLI_ELECTRON_LOG"
SCRIPT
    chmod +x "$missing_cli_workspace/electron"
    cat > "$missing_cli_workspace/codex-update-manager" <<'SCRIPT'
#!/usr/bin/env bash
    printf '%s\n' "$*" >> "$CODEX_MISSING_CLI_UPDATE_MANAGER_LOG"
    exit 1
SCRIPT
    chmod +x "$missing_cli_workspace/codex-update-manager"
    cat > "$missing_cli_workspace/notify-send" <<'SCRIPT'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$CODEX_MISSING_CLI_NOTIFY_LOG"
exit 1
SCRIPT
    chmod +x "$missing_cli_workspace/notify-send"
    cat > "$missing_cli_start" <<'SCRIPT'
#!/bin/bash
set -euo pipefail
CODEX_LINUX_APP_ID=codex-desktop
CODEX_LINUX_APP_DISPLAY_NAME=Codex
CODEX_LINUX_WEBVIEW_PORT=${CODEX_WEBVIEW_PORT:-5175}
SCRIPT
    cat "$REPO_DIR/launcher/start.sh.template" >> "$missing_cli_start"
    chmod +x "$missing_cli_start"

    env -i \
        PATH="$missing_cli_workspace:/usr/bin:/bin" \
        HOME="$missing_cli_home" \
        XDG_CACHE_HOME="$missing_cli_workspace/cache" \
        XDG_CONFIG_HOME="$missing_cli_workspace/config" \
        XDG_STATE_HOME="$missing_cli_workspace/state" \
        XDG_RUNTIME_DIR="$missing_cli_workspace/runtime" \
        TMPDIR="$missing_cli_long_tmp" \
        CODEX_WEBVIEW_PORT="$missing_cli_port" \
        CODEX_MISSING_CLI_ELECTRON_LOG="$missing_cli_electron_log" \
        CODEX_MISSING_CLI_UPDATE_MANAGER_LOG="$missing_cli_update_manager_log" \
        CODEX_MISSING_CLI_NOTIFY_LOG="$missing_cli_notify_log" \
        "$missing_cli_start" >"$missing_cli_stdout" 2>"$missing_cli_stderr" \
        || fail "Launcher must reach Electron onboarding when the CLI is missing; stdout=$(cat "$missing_cli_stdout") stderr=$(cat "$missing_cli_stderr")"
    ...
    case "$missing_cli_short_tmpdir" in
        /tmp/codex-desktop-$(id -u).??????) ;;
        *) fail "Launcher must use a secure mktemp short TMPDIR, got: $missing_cli_short_tmpdir" ;;
    esac
    assert_contains "$missing_cli_electron_log" "TMPDIR_MODE=700"
    assert_contains "$missing_cli_electron_log" "TMPDIR_TYPE=private-directory"
    [ ! -e "$missing_cli_short_tmpdir" ] || fail "Launcher must clean up generated short TMPDIR: $missing_cli_short_tmpdir"
    assert_contains "$missing_cli_update_manager_log" "prompt-install-cli --print-path"
    assert_contains "$missing_cli_notify_log" "Codex CLI is not installed yet. Opening setup without CLI-backed features."
    ...
    assert_contains "$REPO_DIR/launcher/start.sh.template" "WEBVIEW_MONITOR_PID"
    assert_contains "$REPO_DIR/launcher/start.sh.template" "Reusing webview server pid="
    assert_contains "$REPO_DIR/launcher/start.sh.template" "Webview monitor restarting missing webview server"
...
monitor_body = source.split("start_webview_server_monitor() {", 1)[1].split("clear_stale_pid_file() {", 1)[0]
...
if 'RUNNING_APP_PID="$(find_running_app_pid)" || RUNNING_APP_PID="$(discover_running_app_pid)"' not in detect_body:
    raise SystemExit("detect_warm_start must recover a live app even when its launch socket or pid file is missing")
if 'pid_matches_launch_instance "$pid"' not in source:
    raise SystemExit("running-app discovery must match the current launcher instance identity")
if 'actual_instance="$(tr \'\\0\' \'\\n\' < "/proc/$pid/environ"' not in source:
    raise SystemExit("launcher instance matching must read CODEX_LINUX_INSTANCE_ID from the process environment")
if 'exec {LAUNCH_LOCK_FD}>"$LAUNCH_LOCK_FILE"' not in source or 'flock -w "${CODEX_LINUX_LAUNCH_LOCK_TIMEOUT_SECONDS:-60}"' not in source:
    raise SystemExit("launcher startup must be serialized with a bounded per-instance flock")
if source.index('acquire_launch_lock\nreconcile_runtime_state') > source.index('trap cleanup_launcher EXIT'):
    raise SystemExit("launcher must acquire its startup lock before reconciling runtime state")
...
if 'release_launch_lock' not in launch_body:
    raise SystemExit("launch_electron must release the startup lock after Electron is discoverable")
if 'unset ELECTRON_RUN_AS_NODE\n        release_launch_lock\n        "$SCRIPT_DIR/electron"' not in launch_body:
...
if 'cleanup_electron_session_descendants' not in launch_body:
    raise SystemExit("cold Electron exit must clean up surviving app-server and plugin descendants")
if 'start_webview_server_monitor' not in launch_body:
    raise SystemExit("cold Electron launches must start the webview monitor")
if launch_body.index('start_webview_server_monitor') > launch_body.index('wait "$ELECTRON_PID"'):
    raise SystemExit("webview monitor must start before waiting for Electron")
if '(\n        trap - EXIT' not in monitor_body:
    raise SystemExit("webview monitor must not inherit the launcher EXIT cleanup trap")
if 'kill -0 "$ELECTRON_PID"' not in monitor_body:
    raise SystemExit("webview monitor must be tied to the Electron lifetime")
if 'if webview_port_is_open; then' not in monitor_body or 'leaving it untouched' not in monitor_body:
...
if '"$user_data_dir/Code Cache"' not in source or 'web-cache-build.sha256' not in source:
    raise SystemExit("web cache invalidation must be scoped to disposable cache directories and a build marker")
if not re.search(r'run_packaged_runtime_prelaunch\s+log_phase "packaged_prelaunch"\s+invalidate_electron_web_cache_on_build_change\s+log_phase "web_cache_reconciled"\s+ensure_webview_server', runtime_body):
    raise SystemExit("cold start must invalidate stale patched web assets before starting the webview server")
...
if not re.search(r'if needs_cold_start; then\s+clear_bundled_marketplace_tmp_cache\s+# The runtime marketplace is populated asynchronously.*?monitor_bundled_marketplace_tmp_permissions\s+sync_browser_use_bundled_plugin_cache\s+sync_chrome_bundled_plugin_cache\s+sync_computer_use_bundled_plugin_cache\s+run_cold_start_hooks\s+fi', runtime_body, re.S):
    raise SystemExit("bundled marketplace cleanup, plugin sync, and cold-start hooks must run only on cold start")
if not re.search(r'if \[ "\$\{CODEX_LINUX_SYNC_BUNDLED_PLUGINS_ONLY:-0\}" = "1" \]; then\s+export_packaged_runtime_env\s+clear_bundled_marketplace_tmp_cache\s+# Keep this sync-only path aligned with the cold-start cache setup below\.\s+monitor_bundled_marketplace_tmp_permissions\s+sync_browser_use_bundled_plugin_cache\s+sync_chrome_bundled_plugin_cache\s+sync_computer_use_bundled_plugin_cache\s+log_phase "bundled_plugin_sync_only"\s+release_launch_lock\s+exit 0\s+fi', runtime_body, re.S):
    raise SystemExit("sync-only bundled plugin path must refresh plugin caches, release the startup lock, and exit before warm/cold launch")
...
if 'if live_app_pid="$(find_running_app_pid)" || live_app_pid="$(discover_running_app_pid)"; then' not in reconcile_body:
    raise SystemExit("reconcile_runtime_state must recover and preserve a live app without relying on a launch socket")
...
PY
    ...
    assert_contains "$REPO_DIR/SUPPORTED.md" "CI package jobs are validation jobs"
...
assertCacheLinks({
  body: functionBody("sync_computer_use_bundled_plugin_cache", "resolve_browser_use_runtime_env"),
  plugin: "computer-use",
});
NODE
}
...
test_browser_use_node_repl_fallback_runtime() {
    ...
    assert_contains "$install_dir/resources/plugins/openai-bundled/plugins/browser/scripts/browser-client.mjs" "codexLinuxNodeReplCapabilityGuard"
    assert_not_contains "$install_dir/resources/plugins/openai-bundled/plugins/browser/scripts/browser-client.mjs" 'nodeRepl;return e?.config==null'
    ...
    assert_not_contains "$install_dir/resources/plugins/openai-bundled/plugins/browser/scripts/browser-client.mjs" 'console.warn("codexLinuxSiteStatusAllowlistFallback"'
...
test_browser_use_node_repl_capability_guard() {
    ...
    local client="$workspace/browser-client.mjs"
    local drifted_client="$workspace/browser-client-drifted.mjs"
...
JS
    ...
    (
        ...
        # shellcheck disable=SC1091
        source "$REPO_DIR/scripts/lib/bundled-plugins.sh"
        patch_browser_use_node_repl_capability_guard "$client"
        patch_browser_use_node_repl_capability_guard "$client"
    )

    assert_contains "$client" "codexLinuxNodeReplCapabilityGuard"
    assert_contains "$client" 'typeof e.createElicitation!="function"'
    assert_contains "$client" 'typeof e.fetch!="function"'
    assert_not_contains "$client" 'e?.config==null'
    [ "$(grep -o "codexLinuxNodeReplCapabilityGuard" "$client" | wc -l)" = "1" ] \
        || fail "Expected Browser Use nodeRepl capability patch to be idempotent"

    cat > "$drifted_client" <<'JS'
function Me(){let e=globalThis.nodeRepl;return e?.config===void 0?void 0:e}function li(){throw new Error("Browser security unavailable outside node repl")}
JS
    if (
        warn() { echo "[WARN] $*" >&2; }
        info() { echo "[INFO] $*" >&2; }
        # shellcheck disable=SC1091
        source "$REPO_DIR/scripts/lib/bundled-plugins.sh"
        patch_browser_use_node_repl_capability_guard "$drifted_client"
    ); then
        fail "Expected recognizable Browser Use nodeRepl guard drift to fail closed"
    fi
}
...
test_browser_plugin_renamed_upstream_staging() {
    ...
    assert_contains "$browser_dir/scripts/browser-client.mjs" "codexLinuxNodeReplCapabilityGuard"
    ...
    assert_contains "$browser_dir/docs/playwright.md" "Linux Desktop Evaluate And Wait Notes"
    assert_contains "$browser_dir/skills/control-in-app-browser/SKILL.md" "Linux Desktop Runtime Notes"
    [ "$(sed -n '1p' "$browser_dir/skills/control-in-app-browser/SKILL.md")" = "---" ] \
        || fail "Expected Browser skill YAML frontmatter to stay at the start of the file"
    [ "$(grep -c '^---$' "$browser_dir/skills/control-in-app-browser/SKILL.md")" = "2" ] \
        || fail "Expected Browser skill to contain exactly one YAML frontmatter block"
...
JS
    cat > "$chrome_dir/docs/api.md" <<'MD'
interface DomCUAAPI {
  get_visible_dom(): Promise<unknown>; // Return a filtered DOM with node ids for interactable elements.
}
MD
    cat > "$chrome_dir/docs/playwright.md" <<'MD'
## Playwright
Do not assume `locator(...).selectOption(...)` exists in this environment.
MD
...
test_chrome_plugin_staging() {
    ...
    assert_contains "$chrome_dir/scripts/open-chrome-window.js" "CODEX_CHROME_EXECUTABLE"
    ...
    assert_not_contains "$chrome_dir/scripts/browser-client.mjs" 'console.warn("codexLinuxSiteStatusAllowlistFallback"'
    assert_contains "$chrome_dir/docs/api.md" "get_visible_dom(): Promise<string>"
    assert_contains "$chrome_dir/docs/playwright.md" "Linux Desktop Evaluate And Wait Notes"
...
JS
    ...
    cat > "$browser_client" <<'JS'
import{readFile as wH}from"fs/promises";import{resolve as xH}from"path";import{resolve as dH}from"path";import{homedir as pH,platform as fH}from"os";var Xd=dH(pH(),fH()==="win32"?"AppData\\Local\\Google\\Chrome\\User Data":"Library/Application Support/Google/Chrome");import{ClassicLevel as mH}from"./node_modules/classic-level.mjs";import{resolve as ib}from"path";import{tmpdir as hH}from"os";import{cp as gH,mkdtemp as bH,rm as Bk}from"fs/promises";import{existsSync as yH}from"fs";var Fk=async(t,e)=>{let r=ib(Xd,t,"Local Extension Settings",e);if(!yH(r))return null;let n=await bH(ib(_H(),"codex"));await gH(r,n,{recursive:!0}),await Bk(ib(n,"LOCK"));let o=new mH(n,{createIfMissing:!1,keyEncoding:"utf8",valueEncoding:"utf8"});try{await o.open();let i=await o.get("extensionInstanceId");if(!i)return null;let s=JSON.parse(i);return typeof s!="string"?null:s}finally{await o.close(),await Bk(n,{force:!0,recursive:!0})}},_H=()=>"nodeRepl"in globalThis&&globalThis.nodeRepl?globalThis.nodeRepl.tmpDir:hH();var Lk=async t=>{if(t.type!=="extension"||!t.metadata?.extensionInstanceId||!t.metadata.extensionId)return t;let e=await SH(t.metadata.extensionId,t.metadata.extensionInstanceId);return e?{...t,metadata:{...t.metadata,profileName:e.name,profileIsLastUsed:e.isLastUsed.toString(),profileOrdering:e.orderingIndex.toString()}}:t},SH=async(t,e)=>(await vH(t)).find(o=>o.instanceId===e)||null,vH=async t=>{let e=await EH();return await Promise.all(e.map(async r=>({...r,instanceId:await Fk(r.id,t).catch(n=>(ue(n),null))})))},EH=async()=>{let t=xH(Xd,"Local State"),e=JSON.parse(await wH(t,"utf8"));return e.profile.profiles_order.map((r,n)=>{let o=e.profile.info_cache[r];return o?{id:r,name:o.name,isLastUsed:e.profile.last_used===r,orderingIndex:n,avatarUrl:o.avatar_icon}:null}).filter(r=>!!r)};var TH=5e3,ab=cE(CH.platform()),IH=async(t,{codexSessionId:e})=>{let r=Wt(eE),n=t.filter(i=>i.info.type==="iab"),o=AH(n,e,r);return await Promise.all(n.filter(i=>!o.includes(i)).map(async({api:i})=>i.close())),[...t.filter(i=>i.info.type!=="iab"),...o]},AH=(t,e,r)=>e==null?[]:t.filter(n=>n.info.metadata?.codexSessionId===e&&(r==null||n.info.metadata.codexAppBuildFlavor===r)),$k=async(t,e=[])=>[];
JS
    node "$REPO_DIR/scripts/lib/patch-chrome-plugin.js" "$chrome_dir" >/dev/null 2>&1
    assert_contains "$browser_client" "codexLinuxChromeUserDataDirectories"
    assert_contains "$browser_client" '"BraveSoftware","Brave-Browser"'
    assert_contains "$browser_client" '"google-chrome-beta"'
    assert_contains "$browser_client" 'async(t,e,r=Xd)'
    assert_contains "$browser_client" "instanceId:await Fk(o.id,t,r)"
    assert_contains "$browser_client" "codexLinuxRankBrowserBackends"
}
...
test_user_local_prepare_build_repo_overlays_committed_local_changes() {
    ...
    local managed_repo="$workspace/xdg-data/codex-desktop-control/managed-repo"
...
EOF
    ...
    (
        ...
        # shellcheck disable=SC1091
        source "$REPO_DIR/contrib/user-local-install/files/.local/lib/codex-desktop-control/common.sh"
...
test_user_local_install_preserves_persisted_x11_preference_on_refresh() {
    ...
    local preference_file="$config_home/codex-desktop-control/user-local.env"
