...
const PKEXEC_AUTHORIZATION_TIMEOUT: Duration = Duration::from_secs(10);
...
fn set_idle_without_upstream_candidate(
    state: &mut PersistedState,
    paths: &RuntimePaths,
) -> Result<()> {
    if let Some(candidate) = state.candidate_version.take() {
        state
            .notified_events
            .retain(|event| !event.ends_with(&candidate));
    }
...
async fn run_check_cycle(
    ...
    if let Err(error) = result {
        mark_failed_and_persist(state, paths, format!("{error:#}"))?;
        maybe_prune_workspace_cache(&config.workspace_root, state);
...
async fn trigger_install(
    ...
    let output = match install::pkexec_output_with_timeout(
        install::pkexec_command(&current_exe, package_path),
        PKEXEC_AUTHORIZATION_TIMEOUT,
    ) {
        Ok(Some(output)) => output,
        Ok(None) => {
            warn!("graphical package authorization timed out");
            return hand_off_install_authorization(
                state,
                paths,
                &current_exe,
                package_path,
                Some("graphical package authorization timed out".to_string()),
            );
        }
        Err(error) => {
            warn!(?error, "failed to launch graphical package authorization");
            return hand_off_install_authorization(
                state,
                paths,
                &current_exe,
...
fn manual_install_required_message(package_path: &Path) -> String {
    format!(
        "System authorization could not be opened automatically. Run this from a terminal after closing Codex Desktop: {}",
...
fn manual_install_command(package_path: &Path) -> String {
    ...
    format!(
        "pkexec /usr/bin/codex-update-manager {subcommand} --path {}",
...
fn print_manual_install_required(package_path: &Path) {
    println!("Manual install required: system authorization could not be opened automatically.");
...
fn defer_install_for_terminal_authorization(
    state: &mut PersistedState,
    paths: &RuntimePaths,
    terminal_path: &Path,
) -> Result<()> {
    ...
    if let Some(event_key) = install_auth_required_event_key(state) {
        if state.notified_events.insert(event_key) {
            let _ = notify::send(
                "Finish installing Codex Desktop",
                "An authorization terminal was opened. Complete the system prompt there to install the ready update.",
            );
        }
    }
    ...
    persist_state(paths, state)?;
    println!(
        "Opened {} for package-install authorization.",
        terminal_path.display()
    );
    Ok(())
}

fn hand_off_install_authorization(
    state: &mut PersistedState,
    paths: &RuntimePaths,
    current_exe: &Path,
    package_path: &Path,
    pkexec_error: Option<String>,
) -> Result<()> {
    match install::launch_terminal_install(current_exe, package_path) {
        Ok(Some(terminal_path)) => {
            defer_install_for_terminal_authorization(state, paths, &terminal_path)
        }
        Ok(None) => {
            defer_install_for_manual_auth(state, paths, package_path)?;
            maybe_send_manual_install_required_notification(true);
        ...
        Err(terminal_error) => {
            warn!(
                ?terminal_error,
                ?pkexec_error,
                "terminal authorization fallback failed"
            );
            defer_install_for_manual_auth(state, paths, package_path)?;
            maybe_send_manual_install_required_notification(true);
            print_manual_install_required(package_path);
...
"#,
    ...
    #[test]
    fn unchanged_upstream_clears_transient_candidate_state() -> Result<()> {
        let temp = tempfile::tempdir()?;
        let paths = test_paths(temp.path());
        paths.ensure_dirs()?;

        let package_path = temp.path().join("candidate.pkg.tar.zst");
        std::fs::write(&package_path, b"pkg")?;

        let mut state = PersistedState::new(true);
        state.status = UpdateStatus::BuildingPackage;
        state.candidate_version = Some("2026.06.10.163205+58ab7267".to_string());
        state.artifact_paths.package_path = Some(package_path);
        state.error_message = Some("stale build error".to_string());
        state
            .notified_events
            .insert("update_detected:2026.06.10.163205+58ab7267".to_string());
        state.notified_events.insert("cli_missing".to_string());

        set_idle_without_upstream_candidate(&mut state, &paths)?;
    ...
    fn ready_update_auto_install_waits_for_app_exit_when_app_is_running() -> Result<()> {
        ...
        assert!(!state
            .notified_events
            .contains("install_auth_required:2999.03.25.010203+deadbeef"));
        Ok(())
    }
    ...
    #[test]
    fn install_ready_waits_when_app_is_running_without_guessing_auth_agent() -> Result<()> {
        let _env_guard = crate::test_util::env_lock();
        let runtime = tokio::runtime::Runtime::new()?;
        let temp = tempfile::tempdir()?;
        let paths = RuntimePaths {
    ...
    fn manual_install_command_selects_package_kind_and_quotes_path() {
        assert_eq!(
            ...
            "pkexec /usr/bin/codex-update-manager install-pacman --path '/tmp/codex update.pkg.tar.zst'"
        );
        assert_eq!(
            manual_install_command(Path::new("/tmp/codex'update.deb")),
            "pkexec /usr/bin/codex-update-manager install-deb --path '/tmp/codex'\\''update.deb'"
        );
    }
    ...
    fn clear_install_auth_required_event_keeps_unrelated_notifications() -> Result<()> {
        ...
        assert!(!state
            .notified_events
            .contains("install_auth_required:2026.04.28.082247+abcdef12"));
        assert!(state
            .notified_events
    ...
    #[test]
    fn failed_update_is_cleared_when_candidate_was_installed_manually() -> Result<()> {
        let temp = tempfile::tempdir()?;
        let paths = test_paths(temp.path());
        paths.ensure_dirs()?;
        ...
        let mut state = PersistedState::new(true);
        state.status = UpdateStatus::Failed;
        state.installed_version = "2026.04.28.082247+abcdef12-1".to_string();
        state.candidate_version = Some("2026.04.28.082247+abcdef12".to_string());
        state.error_message = Some("install.sh failed during local rebuild".to_string());
        state
            .notified_events
            .insert("build_failed:2026.04.28.082247+abcdef12".to_string());
